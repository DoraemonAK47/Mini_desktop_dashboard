...
void loop() {
    if(P1 == "Class%201"){
      ...
      if (currentMillis - previousMillis >= interval) { // send per 5s
        ...
        Serial.print("Room_state: ");
        Serial.println(Message1);
        client.publish(topic_send, Message1);
      }
  }else if(P1 == "Class%202"){
      if (!client.connected()) {
        reconnect();
        }
      client.loop();
      currentMillis = millis();
      if (currentMillis - previousMillis >= 2000) { // send per 5s
        previousMillis = currentMillis;
        Serial.print("Room_state: ");
        Serial.println(Message3);
        client.publish(topic_send, Message3);
      }
  }
}
    ...
    if (cmd.length()>0) {
      ...
      if (cmd=="your cmd") {
        ...
        if (P1=="4") {
          ...
          ledcAttachChannel(P1.toInt(), 5000, 8, 9);
        ...
        if (P1!=""&P1!="stop") Serial.println(P1);  //output P1 only
